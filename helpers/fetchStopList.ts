...
export default async function fetchStopList() {
	...
	if (response && response.ok) {
		// return body as GetStopsReponse
		const allStops = (await response.json()) as GetStopsReponse;

		// convert the original stops to Option[]
		const parent_stops = allStops.stops.map((stop) => ({ title: stop.stop_name, value: stop.stop_id }));
		...
		const boarding_points = stops.map((stop: StopPoint) => ({ title: stop.stop_name, value: stop.stop_id }));
